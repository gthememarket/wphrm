...
if(isset($_REQUEST['tab']) && $_REQUEST['tab']=="view_employee"){
	...
	$user_role = hrmgt_get_user_role($empdata->ID);
		...
		<div class="col-md-6 right_side">
			<table class="table">
				<tbody>
					...
					<tr>
					<?php
					$current_role = hrmgt_get_user_role(get_current_user_id());							
					if($current_role=="manager") { 
						if($user_role == 'employee'){ ?>
							<td><a href="?hr-dashboard=user&page=user&tab=add_user&user_type=manager&action=edit&id=<?php  print $user_id; ?>" class="btn btn-primary"> <?php _e(' Edit Experience Letter','hrmgt') ?></a></td> 
					<?php } } ?>	
				<?php if($current_role=='employee' && $empdata->ID == get_current_user_id() || $current_role == 'manager'){		if($user_role=='employee'){
				?>			
	...
	<div class="col-md-4 member_right">	
			...
			<table class="table">
				<tbody>						
					...
					<tr>
						<th scope="row"><i class="fa fa-leaf" aria-hidden="true"></i> <?php _e('Department','hrmgt') ?>  </th>
						<td scope="row"><span class="txt_color"><?php print hrmgt_get_department_name(get_user_meta($user_id,'department',true));?> </span></td>
					</tr>
					<?php if($designation){?>
					<tr>
						<th scope="row"><i class="fa fa-child" aria-hidden="true"></i><?php _e('Designation','hrmgt') ?>  </th>
